/* ===========================================================================
   ...
   of them and the path tells them apart:
       ...
       /avo-basics/modules/add-offset.html
       /attribute_quiz/
   ...
   THE QUERY STRING IS DELIBERATELY DROPPED. Every module writes the position of
   every control into the address bar, so that a particular setup can be handed
   out as a link. Left alone, GoatCounter reports the path AND the query string,
   which would file each visit under its own row: one module of avo-basics has
   about eleven billion reachable URLs, so five hundred visits would appear as
   five hundred rows of one view each and the module's total would read zero.
   The `path` setting below strips it, which is GoatCounter's own supported way
   of doing this. Every repository needs it, because they all put state in the
   URL. Removing it does not break anything visibly — it quietly makes the
   dashboard useless, which is worse.
...
(function () {
  ...
  // Report the page, not the settings. See the note at the top of this file:
  // without this, every slider position becomes its own row. Must be set before
  // the script below loads, which is why it is here and not after.
