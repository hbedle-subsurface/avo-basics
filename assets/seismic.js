/* ===========================================================================
   ...
   Shared with the companion module sets, so it carries a few helpers this
   set does not use. Keep it in step across the repositories rather than
   trimming it per site.
...
const SEIS = (function () {
  ...
  /* Hilbert transform of a single trace, via the analytic signal: zero the
     ...
     the result is the quadrature. Not used by this set; kept because the
     companion sets build instantaneous attributes from the analytic trace. */
  function hilbert(x) {
    const n0 = x.length;
    let n = 1;
    while (n < 2 * n0) n *= 2;
