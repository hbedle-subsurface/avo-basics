...
/* The site must say what it records, or the disclosure is a lie of omission.
   The wording differs between the module footers and the index, which makes the
   point in its About section instead, so match the substance rather than one
   fixed sentence. */
const DISCLOSURE = /(only|one) thing recorded is that\s+(a|the)\s+page\s+was\s+opened/i;
const missingNotice = pages.filter((p) =>
  !DISCLOSURE.test(fs.readFileSync(path.join(ROOT, p), 'utf8')));
